...
FluContentPage{
    ...
    Component.onCompleted: {
        ...
        for (var i = 0; i < patients.length; i++) {
            ...
            else 
            {
                ...
                appointment_id.push(patientMap["appointment_id"])
            }   
        }
    ...
    Component {
        ...
        Item {
            RowLayout {
                ...
                FluFilledButton {
                    text: qsTr("诊断")
                    // 通过 property 绑定患者用户名和挂号单号
                    property string patient_username: options.patient_username
                    property string appointment_id: options.appointment_id

                    onClicked: {
                        // 创建一个新窗口的组件
                        console.log("Clicked patient_username: " + patient_username)
                        console.log("Clicked appointment_id: " + appointment_id)
                        var newWindowComponent = Qt.createComponent("Diagnose_inwindow.qml");
    ...
    function genTestObject(i) {
        // 生成表格每一行的数据对象，并确保传递正确的患者信息和预约信息
        return {
            name: patient_name[i],
            age: patient_age[i],
            ...
            action: table_view.customItem(com_action, {
                patient_username: patient_username[i],
                appointment_id: appointment_id[i]
            }),
            _minimumHeight: 50,
            _key: FluTools.uuid()
        };
